...
elif args.dataset.startswith('treadmill'):
    from common.treadmill_dataset import TreadmillDataset
    dataset = TreadmillDataset(dataset_path)
else:
...
def run_training_and_evaluation():
    ...
    model_pos_train =  MixSTE2(num_frame=receptive_field, num_joints=num_joints, num_joints_out=num_joints_out, in_chans=2, embed_dim_ratio=args.cs, depth=args.dep,
            num_heads=8, mlp_ratio=2., qkv_bias=True, qk_scale=None,drop_path_rate=0.1)

    model_pos =  MixSTE2(num_frame=receptive_field, num_joints=num_joints, num_joints_out=num_joints_out, in_chans=2, embed_dim_ratio=args.cs, depth=args.dep,
    ...
    if args.resume or args.evaluate or args.finetune:
        chk_filename = os.path.join(args.checkpoint,
                                    args.resume if args.resume else
                                    args.evaluate if args.evaluate else
                                    args.finetune)
    ...
    if args.csv_export:
        ...
        for subject in subjects_test:
            print('Predicting Subject', subject)
            for action in dataset[subject].keys():
                print('Action', action)
                input_keypoints = keypoints[subject][action].copy()
                ...
                flat = prediction.reshape((len(prediction), dataset.skeleton().num_joints() * 3))

                output_folder = os.path.join(args.csv_output, args.evaluate.split('.')[0], subject)
                output_file = os.path.join(output_folder, action + '.csv')
                print('Exporting joint positions to ', output_file)
                os.makedirs(output_folder, exist_ok=True)

                np.savetxt(output_file, flat, delimiter=';')
...
if args.leave_one_subject_out:
    # ignore given test/train subjects
    subjects = dataset.subjects()
    for s in subjects:
        subjects_train = [x for x in subjects if x != s]
        subjects_test = [s]
        # todo: add support for semi-supervised learning
        subjects_semi = [] if not args.subjects_unlabeled else args.subjects_unlabeled.split(',')

        print('Training on subjects {} and testing on subject {}'.format(subjects_train, subjects_test))
        run_training_and_evaluation()
else:
    subjects_train = args.subjects_train.split(',')
    subjects_semi = [] if not args.subjects_unlabeled else args.subjects_unlabeled.split(',')
    subjects_test = args.subjects_test.split(',')
    print('Training on subjects {} and testing on subjects {}'.format(subjects_train, subjects_test))
    run_training_and_evaluation()
